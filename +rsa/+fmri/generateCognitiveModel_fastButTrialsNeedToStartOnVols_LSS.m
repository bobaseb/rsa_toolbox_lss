function [cognitiveX,BVstimProt,standardIndexSequence,hirf_ms,trialImpulseX_TRvol2]=generateCognitiveModel_fastButTrialsNeedToStartOnVols_LSS(sequence,nTRvols_final,nSkippedVols,monitor,scaleTrialResponseTo1,simulationOptions)
...
% modified by Sebastian Bobadilla Suarez to model separate trials (cf. LSS
% in Mumford et al., 2012)
...
%% control variables
stimDur_ms = simulationOptions.stimulusDuration*1000;
trialDur_ms = simulationOptions.trialDuration*1000;
TRvol_ms = simulationOptions.TR*1000;
...
nReps = sum(sequence==1); %how many repetitions per stimulus
%% give trial types consecutive integer values
% existingTrialTypeIDsVector=zeros(max(sequence),1);B_true = zeros(simulationOptions.nConditions*nReps,nVox_wholeBrain);
...
trialImpulseX_TRvol2=zeros(durationX_TRvol,nPreds*nReps); %original function doesn't model individual presentations
...
for trialI=1:nTrials
    if standardIndexSequence(trialI)~=nTrialTypes % if not a null trial
        if (trialI-1)*trialDur_TRvol+1-nSkippedVols>0
            ...
            trialImpulseX_TRvol2((trialI-1)*trialDur_TRvol+1-nSkippedVols,trialI)=standardIndexSequence(trialI);
        else
...
trialImpulseX_TRvol = trialImpulseX_TRvol2(:,sum(trialImpulseX_TRvol2)~=0); %takes out null trials, due to bad indexing in previous loop
trialImpulseX_TRvol = trialImpulseX_TRvol>0; %turns all numbers above 0 into 1 (impulse response)
...
if simulationOptions.reduce_design %creates LSS design matrices
    sequence_tmp = sequence(sequence<max(sequence)); %for non-null trials
    msk = sequence_tmp==max(sequence_tmp); %this mask decides which trials are clumped together
    trialImpulseX_TRvol_tmp = [trialImpulseX_TRvol(:,~msk) sum(trialImpulseX_TRvol(:,msk),2)];
    trialImpulseX_TRvol = trialImpulseX_TRvol_tmp;
end
...
for predI=1:size(trialImpulseX_TRvol,2) %just changed nPreds from original loop
    cognitiveX(:,predI)=conv(trialImpulseX_TRvol(:,predI),hemodynamicTrialResponse_TRvol); % c)urrent pred)ictor of h)emodynamic r)esponse (in TRvols)
end

%if ~isempty(nTRvols_final) %decided to leave BOLD tail of last trials for LSS models
