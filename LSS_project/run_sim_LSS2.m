...
simulationOptions.cov_mat = sigma_mat; %cov_mat./round(simulationOptions.cov_mat_df); % scale the covariance matrix
...
for run = 1:simulationOptions.nruns
    ...
    r_s(i) = DATA2.(sprintf('run%d',run)).r1;
    err(i) = immse(tmp_B_true,tmp_B_LSA);
...
if length(simulationOptions.model_list) > 1
    ...
    for run = 1:simulationOptions.nruns
        ...
        r_s(i) = DATA2.(sprintf('run%d',run)).r2;
        err(i) = immse(tmp_B_true,tmp_B_LSS00);
    ...
    for run = 1:simulationOptions.nruns
        ...
        r_s(i) = DATA2.(sprintf('run%d',run)).r3;
        err(i) = immse(tmp_B_true,tmp_B_LSS01);
    ...
    for run = 1:simulationOptions.nruns
        ...
        r_s(i) = DATA2.(sprintf('run%d',run)).r4;
        err(i) = immse(tmp_B_true,tmp_B_LSS10);
...
if length(simulationOptions.model_list) > 4
    ...
    for run = 1:simulationOptions.nruns
        ...
        r_s(i) = DATA2.(sprintf('run%d',run)).r5;
        err(i) = immse(tmp_B_true,tmp_B_LSS02);
    ...
    for run = 1:simulationOptions.nruns
        ...
        r_s(i) = DATA2.(sprintf('run%d',run)).r6;
        err(i) = immse(tmp_B_true,tmp_B_LSS11);
    ...
    for run = 1:simulationOptions.nruns
        ...
        r_s(i) = DATA2.(sprintf('run%d',run)).r7;
        err(i) = immse(tmp_B_true,tmp_B_LSS20);
    ...
    for run = 1:simulationOptions.nruns
        ...
        r_s(i) = DATA2.(sprintf('run%d',run)).r8;
        err(i) = immse(tmp_B_true,tmp_B_LSS12);
    ...
    for run = 1:simulationOptions.nruns
        ...
        r_s(i) = DATA2.(sprintf('run%d',run)).r9;
        err(i) = immse(tmp_B_true,tmp_B_LSS21);
    ...
    for run = 1:simulationOptions.nruns
        ...
        r_s(i) = DATA2.(sprintf('run%d',run)).r10;
        err(i) = immse(tmp_B_true,tmp_B_LSS22);
    ...
    ridge_betas = inv(X'X + lambda*eye(ndims))*(X'Y + lambda*B_LSS00);
