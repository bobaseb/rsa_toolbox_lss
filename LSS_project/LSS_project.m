...
%addpath('/mnt/love12/libsvm-3.22/matlab');

%% setup model parameters

parjob = 1; %1 if using cluster, runs njobs
njobs = 100; %number of jobs per noise setting, only if running on many cores
...
% smoothing FWHM.
smoothing_kernel = [2.5 2.5 2.5 2.5];
...
% the noise is drawn (?).
simulationOptions.scannerNoiseLevel = 1000; %3000; %10000./1000; % used to be 3000
...
if big_brain==1
    ...
    save_path = ['/media/seb/HD_Numba_Juan/sim_results_' num2str(simulationOptions.nf) 'voxel_smoothing' skernel_str '_noise' num2str(simulationOptions.scannerNoiseLevel) '_' num2str(simulationOptions.nRepititions) 'samps_bigBrain'];
elseif big_brain==0
    ...
    %where to save the simulation results
    save_path = ['/media/seb/HD_Numba_Juan/sim_results_' num2str(simulationOptions.nf) 'voxel_smoothing' skernel_str '_noise' num2str(simulationOptions.scannerNoiseLevel) '_' num2str(simulationOptions.nRepititions) 'samps'];
end
...
trial_duration_list = [2,3,4]; % in secondsdelete(gcp('nocreate'))
...
for trial_duration = 1:length(trial_duration_list)
    for big_sigma = 1:length(big_sigma_list) %for scan_noise = 1:length(scan_noise_list) %
        ...
        if parjob==0
            ...
            simulationOptions.scannerNoiseLevel = 1000;
            ...
            parfor i=1:njobs
                %[accs, sigma_est, DATA] = run_sim_LSS(simulationOptions);
            ...
            cd(save_path)
            %save(['bsig',num2str(simulationOptions.big_sigma),'tsig',num2str(simulationOptions.trial_sigma),'isi', num2str(simulationOptions.trialDuration),'.mat'],'SIMS','all_accs','all_sigma_est','-v7.3')
